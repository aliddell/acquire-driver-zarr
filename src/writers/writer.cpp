                          ...
                          std::vector<file>& files) noexcept
{
    ...
    if (!create_z_dirs_(n_c, n_z)) {
        return false;
    }

    if (!create_y_dirs_(n_c, n_z, n_y)) {
        return false;
    }

    const auto n_files = n_c * n_z * n_y * n_x;
...
bool
zarr::FileCreator::create_y_dirs_(int n_c, int n_z, int n_y) noexcept
{
    std::latch latch(n_c * n_z * n_y);
    std::atomic<bool> failure{ false };
    for (auto c = 0; c < n_c; ++c) {
        for (auto z = 0; z < n_z; ++z) {
            for (auto y = 0; y < n_y; ++y) {
                zarr_->push_to_job_queue(
                  std::move([base = base_dir_, c, z, y, &latch, &failure](
                              std::string& err) {
                      bool success = false;
                      try {
                          const auto path = base / std::to_string(c) /
...
bool
zarr::FileCreator::create_z_dirs_(int n_c, int n_z) noexcept
{
    std::latch latch(n_c * n_z);
    std::atomic<bool> failure{ false };
    for (auto c = 0; c < n_c; ++c) {
        for (auto z = 0; z < n_z; ++z) {
            zarr_->push_to_job_queue(std::move(
              [base = base_dir_, c, z, &latch, &failure](std::string& err) {
                  bool success = false;
                  try {
                      const auto path =
                        base / std::to_string(c) / std::to_string(z);
                      if (fs::exists(path)) {
                          EXPECT(fs::is_directory(path),
                                 "%s must be a directory.",
                                 path.c_str());
...
zarr::Writer::close_files_()
{
    ...
    // FIXME (aliddell): this should happen BEFORE closing files
